...
std::uint8_t quantize(float value, float scale) {
    if (!std::isfinite(value) || !(scale > 0.0f)) throw std::invalid_argument("invalid quantization input");
    const int signed_q = std::clamp(static_cast<int>(std::lrint(value / scale)), -8, 7);
    return static_cast<std::uint8_t>(signed_q + 8);
}

EncodedStream encode_symbols(const std::vector<std::uint8_t>& symbols, std::uint32_t block_size) {
    if (symbols.empty() || block_size == 0) throw std::invalid_argument("empty stream or block size");
    EncodedStream out;
    out.block_size = block_size;
    out.lut.resize(16);
    for (int symbol = 0; symbol < 16; ++symbol) out.lut[symbol] = symbol | (4 << 8);

    const std::uint64_t bit_count = static_cast<std::uint64_t>(symbols.size()) * 4u;
    ...
    for (std::size_t i = 0; i < symbols.size(); ++i) {
        if (i % block_size == 0) out.block_offsets.push_back(static_cast<std::int32_t>(i * 4u));
        const std::uint64_t bit = static_cast<std::uint64_t>(i) * 4u;
        const std::uint32_t word = static_cast<std::uint32_t>(bit >> 5u);
        const std::uint32_t shift = 28u - static_cast<std::uint32_t>(bit & 31u);
        out.words[word] |= static_cast<std::uint32_t>(symbols[i] & 0x0fu) << shift;
    }
    return out;
}
...
std::vector<std::uint8_t> decode_symbols(const EncodedStream& stream, std::uint64_t symbol_count) {
    if (stream.max_code_length != 4 || stream.lut.size() != 16) throw std::invalid_argument("unsupported fixture codebook");
    std::vector<std::uint8_t> result(static_cast<std::size_t>(symbol_count));
    for (std::uint64_t i = 0; i < symbol_count; ++i) {
        const std::uint64_t bit = i * 4u;
        const std::uint32_t word = static_cast<std::uint32_t>(bit >> 5u);
        const std::uint32_t shift = 28u - static_cast<std::uint32_t>(bit & 31u);
        if (word >= stream.words.size()) throw std::invalid_argument("truncated fixture stream");
        result[static_cast<std::size_t>(i)] = static_cast<std::uint8_t>((stream.words[word] >> shift) & 0x0fu);
    }
    return result;
}
...
EncodedKvPage encode_int4_page(const std::vector<float>& keys,
    ...
    for (std::uint32_t d = 0; d < head_dim; ++d) {
        ...
        page.k_scales[d] = maximum == 0.0f ? 1.0f : maximum / 7.0f;
    }
    for (std::uint32_t t = 0; t < token_count; ++t) {
        ...
        page.v_scales[t] = maximum == 0.0f ? 1.0f : maximum / 7.0f;
    }

    for (std::uint32_t t = 0; t < token_count; ++t) {
        for (std::uint32_t d = 0; d < head_dim; ++d) {
            const std::size_t index = static_cast<std::size_t>(t) * head_dim + d;
            k_symbols[index] = quantize(keys[index], page.k_scales[d]);
            v_symbols[index] = quantize(values[index], page.v_scales[t]);
            page.dequantized_k[index] = (static_cast<int>(k_symbols[index]) - 8) * page.k_scales[d];
            page.dequantized_v[index] = (static_cast<int>(v_symbols[index]) - 8) * page.v_scales[t];
        }
    }

    page.k = encode_symbols(k_symbols, block_size);
    page.v = encode_symbols(v_symbols, block_size);
    validate_page(page);
    return page;
}
...
void validate_page(const EncodedKvPage& page) {
    ...
    if (page.token_count == 0 || page.head_dim == 0 || page.kv_head > std::numeric_limits<std::uint32_t>::max() ||
        ...
        page.k.lut.size() != 16 || page.v.lut.size() != 16) {
